...
class StreamService {
  ...
  validateDirectory() {
    const hlsOutputDir = config.streamDirectory.hls.output.dir;
    this.checkDirectoryExists(hlsOutputDir) 
      ? this.cleanDirectory(hlsOutputDir) 
      : this.createDirectory(hlsOutputDir);
  }
  ...
  async findFile(directory, fileName) {
  ...
  checkFileStreamExists(hlsOutputDir, hlsOutputFileName) {
    ...
    if (!config.ffmpegConfig.codecs || !config.ffmpegConfig.codecs.video || !config.ffmpegConfig.codecs.audio) {
    ...
    this.data.intervalId = setInterval(async () => {
      checkCount++;
      const fileStreamExists = await this.findFile(hlsOutputDir, hlsOutputFileName);
  ...
  startStreamConversion(camera) {
    ...
    const process = this.processFFmpegStream(config.ffmpegConfig, camera);
    process.run();
